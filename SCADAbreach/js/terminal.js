...
writeFile('/var/log/historian/historian.log',
  ...
  `${new Date().toISOString()} some other normal log lines...\n`
);
...
function renderPrompt(cmdText = '') {
  ...
  if (__lastPromptEl) {
    __lastPromptEl.textContent = `${currentUser()}@${hostname}:${cwdStr}${promptChar()} ${cmdText}`;
if (typeof input !== 'undefined' && input) input.placeholder = `${currentUser()}@${hostname}:${cwdStr}${promptChar()} `;
    return __lastPromptEl;
  }

  const el = document.createElement('div');
  el.textContent = `${currentUser()}@${hostname}:${cwdStr}${promptChar()} ${cmdText}`;
  el.className = 'prompt';
  body.appendChild(el);
  body.scrollTop = body.scrollHeight;
  return el;
...
function cmd_clear(){
  clear();           // czyści ekran
  __lastPromptEl = renderPrompt(); // wstaw świeży prompt
}
...
if (__customCmds[name]) { __customCmds[name](args); return; }
  ...
  input.addEventListener('keydown', (e) => {
    if(e.key === 'Enter'){
      ...
      const pathParts = [];
      let cur = cwd;
      while(cur && cur !== root){ pathParts.unshift(cur.name); cur = cur.parent; }
      const cwdStr = '/' + pathParts.join('/');
      renderPrompt(cmd);
__lastPromptEl = null;
